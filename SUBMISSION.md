...
**Not built:** any Bitget Agent Hub, MCP, or Playbook integration, any paper-trading evaluation of CDE's actual research question, and any multi-model or multi-configuration comparison — see [README.md § What would be next](README.md#what-would-be-next).

**Separately validated, not part of CDE's result:** real authenticated Bitget Demo (paper-trading) order execution — a `BTCUSDT` buy and sell both filled successfully. We then tested whether this could extend to CDE's actual rToken instruments and found Bitget's Demo service explicitly rejects RWA/rToken orders platform-wide (`code 25200`, reproduced across four rToken symbols and two order types). We did not substitute `BTCUSDT` for the real experiment. Full evidence: [README.md § Bitget Demo execution](README.md#bitget-demo-execution--validated-separately-and-why-it-isnt-part-of-cdes-result).
...
- **Track/sub-theme fit is imperfect, and that is stated plainly rather than hidden.** The official guide scores Agentic Trading as "50% quantitative + 50% judge," with required materials including a paper-trading log "actually run during competition period, recommended ≥2 weeks." CDE has no such log and no quantitative trading metrics — its quantitative content is the DiD/CI/p-value of a decision-level evaluation, not a Sharpe/return-style trading number. This was not for lack of trying: we directly tested real Bitget Demo execution and confirmed it authenticates and fills orders correctly, then confirmed the platform explicitly refuses orders on the rToken/RWA instrument class CDE's research is about (see [README.md § Bitget Demo execution](README.md#bitget-demo-execution--validated-separately-and-why-it-isnt-part-of-cdes-result)). Open Theme, whose own example direction is agent evaluation/benchmarks, is the closest honest fit; it is not a perfect fit, and no attempt was made here to manufacture paper-trading data to close that gap. Confirm this track/sub-theme choice is still the right call before submitting.
